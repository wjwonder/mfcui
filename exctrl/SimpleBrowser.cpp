...
BEGIN_EVENTSINK_MAP(SimpleBrowser,CWnd)
	...
	ON_EVENT(SimpleBrowser,AFX_IDW_PANE_FIRST,
			 DISPID_NEWWINDOW3,
			 _OnNewWindow3, 
			 VTS_PDISPATCH VTS_PBOOL VTS_UI4 VTS_BSTR VTS_BSTR)
    ON_EVENT(SimpleBrowser,AFX_IDW_PANE_FIRST,
                                       ...
                                       VARIANT_BOOL *Cancel)
{
    ...
    if (lpDisp == _BrowserDispatch) {
		...
		if ((TargetFrameName       != NULL) &&
			...
			frame = CString(V_BSTR(TargetFrameName));
        }
		...
		if ((Headers       != NULL) &&
			...
			headers = CString(V_BSTR(Headers));
        }
...
void SimpleBrowser::_OnNewWindow3(IDispatch **ppDisp, VARIANT_BOOL* Cancel, DWORD dwFlags, BSTR bstrUrlContext, BSTR bstrURL)
{	
	CString strURL;
	CString strUrlContext;
	if (bstrURL != NULL) {
		strURL = (LPCTSTR)bstrURL;
	}
	if(bstrUrlContext != NULL)
	{
		strUrlContext = (LPCTSTR)bstrUrlContext;
	}

	bool cancel = OnNewWindow3(strURL, strUrlContext, dwFlags);
	if(cancel)
	{
		*Cancel = VARIANT_TRUE;
	}
...
bool SimpleBrowser::OnNewWindow3(CString strURL, CString strURLContext, DWORD dwFlags)
{
	bool cancel = false;
	...
	if (parent != NULL) {
		...
		Notification	notification(m_hWnd,GetDlgCtrlID(),NewWindow3);

		notification.URL			= strURL;;
